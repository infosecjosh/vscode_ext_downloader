...
def main():
    ...
    for extension in get_vscode_extensions(session=session):
        ...
        for extension_version_info in extensions_versions:
            ...
            print(extension_name, extension_description, extension_version, extension_artifact_download_url)
            filename = extension_publisher_username.strip() + "." + extension_name + "-" + extension_version + ".vsix"
            print (filename)
            urllib.request.urlretrieve(extension_artifact_download_url, filename)
